...
void pretty_ftoa(char *buf, size_t bufsiz, double d)
{
	...
	if (!hi_is_expo)
	{
		...
		lo = 1;
		ftoa_iter(noexpo, sizeof(noexpo), lo, d, &len, &is_expo);
		if (is_expo || atof(noexpo) != d)
		{
			...
			while (lo + 1 < hi)
			{
				...
				ftoa_iter(noexpo, sizeof(noexpo), cur, d, &len, &is_expo);
				if (is_expo || atof(noexpo) != d)
				{
					lo = cur;
				}
	...
	lo = 1;
	hi = 17;
	ftoa_iter(curbuf, sizeof(curbuf), lo, d, &len, &is_expo);
	if (!is_expo)
	{
	...
	if (atof(curbuf) != d)
	{
		...
		while (lo + 1 < hi)
		{
			...
			ftoa_iter(curbuf, sizeof(curbuf), cur, d, &len, &is_expo);
			if (atof(curbuf) != d)
			{
				lo = cur;
			}
